...
public class Main {
    ...
    public static void registrarUsuario() {
        ...
        // Validaciones generales de los datos del usuario
        String error = UsuarioValidacion.validarDni(dni);
        if (error == null) error = UsuarioValidacion.validarContrasenia(contrasenia);
        if (error == null) error = UsuarioValidacion.validarNombre(nombre);
        if (error == null) error = UsuarioValidacion.validarApellido(apellido);
        if (error == null) error = UsuarioValidacion.validarEmail(email);
        if (error == null && !telefono.trim().isEmpty()) error = UsuarioValidacion.validarTelefono(telefono);
        if (error != null) {
            JOptionPane.showMessageDialog(null, error, "Validacion", JOptionPane.WARNING_MESSAGE);
            return;
        }
